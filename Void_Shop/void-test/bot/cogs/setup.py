...
Enthält !role und !Start Befehle mit vollem Rate-Limit Schutz (3.0s Pause)
und dynamischem Berechtigungssystem für alle 23 Rollen im korrekten Design.
"""
...
class SetupCog(commands.Cog, name="SetupCog"):
    ...
    def build_overwrites(self, guild, mode):
        ...
        # --- REWARD ROLES (Jetzt alle korrekt mit 𝗩𝗢𝗜𝗗 • Prefix!) ---
        r_bronze = get_r("🥉│ 𝗩𝗢𝗜𝗗 • Bronze Buyer")
        r_silver = get_r("🥈│ 𝗩𝗢𝗜𝗗 • 𝗦𝗶𝗹𝘃𝗲𝗿 𝗕𝘂𝘆𝗲𝗿")
        r_gold = get_r("🥇│ 𝗩𝗢𝗜𝗗 • 𝗚𝗼𝗹ｄ 𝗕ｕ𝘆𝗲𝗿")
        r_diamond = get_r("💎│ 𝗩𝗢𝗜𝗗 • 𝗗𝗶𝗮ｍｏｎ𝗱 𝗕ｕ𝘆𝗲𝗿")

        # --- NOTIFICATION ROLES (Jetzt alle korrekt mit 𝗩𝗢𝗜𝗗 • Prefix!) ---
        r_ping_ann = get_r("📢│ 𝗩𝗢𝗜𝗗 • 𝗔𝗻𝗻ｏ𝘂𝗻𝗰𝗲𝗺𝗲𝗻𝘁 𝗣𝗶𝗻ｇ")
        r_ping_gw = get_r("🎁│ 𝗩𝗢𝗜𝗗 • 𝗚𝗶𝘃𝗲𝗮𝘄𝗮ｙ 𝗣𝗶𝗻ｇ")
        r_ping_prod = get_r("📦│ 𝗩𝗢𝗜𝗗 • 𝗣𝗿ｏ𝗱ｕ𝗰𝘁 𝗣𝗶𝗻𝗴")

        # --- BASE ROLES ---
        r_member = get_r("👥│ 𝗩𝗢𝗜𝗗 • 𝗠ｅｍｂｅ𝗿")
    ...
    async def create_roles_command(self, ctx):
        """
        Erstellt alle 23 Premium-Rollen mit verlässlichem Rate-Limit-Schutz (3.0s Pause),
        ...
        role_colors = {
            ...
            # --- REWARD ROLES (ALLE KORREKT MIT 𝗩𝗢𝗜𝗗 • PREFIX) ---
            "🥉│ 𝗩𝗢𝗜𝗗 • 𝗕𝗿ｏ𝗻ｚｅ 𝗕𝘂𝘆𝗲𝗿": (0xcd7f32, False),       # Bronze
            "🥈│ 𝗩𝗢𝗜𝗗 • 𝗦𝗶𝗹𝘃𝗲𝗿 𝗕𝘂𝘆𝗲𝗿": (0xc0c0c0, False),       # Silver
            "🥇│ 𝗩𝗢𝗜𝗗 • 𝗚𝗼𝗹𝗱 𝗕𝘂𝘆𝗲𝗿": (0xffd700, False),         # Gold
            "💎│ 𝗩𝗢𝗜𝗗 • 𝗗𝗶𝗮ｍｏｎ𝗱 𝗕𝘂𝘆𝗲𝗿": (0xb9f2ff, False),       # Diamond
            
            # --- NOTIFICATION ROLES (ALLE KORREKT MIT 𝗩𝗢𝗜𝗗 • PREFIX) ---
            "📢│ 𝗩𝗢𝗜𝗗 • 𝗔𝗻𝗻ｏ𝘂𝗻𝗰𝗲𝗺𝗲𝗻𝘁 𝗣𝗶𝗻𝗴": (0x7289da, False),  # Discord Blue
            "🎁│ 𝗩𝗢𝗜𝗗 • 𝗚𝗶𝘃𝗲𝗮𝘄𝗮ｙ 𝗣𝗶𝗻𝗴": (0xff4500, False),      # Red-Orange
            "📦│ 𝗩𝗢𝗜𝗗 • 𝗣𝗿ｏ𝗱𝘂𝗰𝘁 𝗣𝗶𝗻𝗴": (0x32cd32, False),       # Lime Green
        ...
        for idx, (role_name, (color_hex, is_admin)) in enumerate(role_colors.items(), 1):
            progress_embed.description = (
                ...
                f"> Aktuell: `{role_name}`\n\n"
                f"> ⚠️ *Hinweis: Discord hat ein strenges Rate-Limit für Rollen. Sollte der Bot hier kurz pausieren (30-60s), bricht er NICHT ab, sondern wartet lediglich das Discord Rate-Limit ab! Bitte einfach laufen lassen.*"
            )
            try:
            ...
            try:
                ...
                # 3.0s Pause schützt uns absolut zuverlässig vor dem harten Discord API Rate-Limit!
