...
class EconomyCog(commands.Cog, name="EconomyCog"):
    ...
    async def redeem_command(self, interaction: discord.Interaction, key: str):
        ...
        embed = EmbedHelper.create_prestige_embed(
            ...
            description=(
                f"> Du hast **{p.get('emoji','')} {p.get('name','dein Produkt')}** freigeschaltet!{extra_info}\n"
                "~~                                                              ~~\n"
    ...
    @app_commands.command(name="flashsale", description="⚡ [Admin] Blitzangebot & Flash-Sale auf Website und Discord starten")
    @app_commands.autocomplete(produkt=_product_autocomplete)
    @app_commands.guild_only()
    async def flashsale_command(self, interaction: discord.Interaction, produkt: str, rabatt: int, minuten: int):
        if not interaction.user.guild_permissions.administrator:
            return await interaction.response.send_message("❌ Nur Admins!", ephemeral=True)
        from bot.cogs.tickets import PRODUCTS
        if produkt not in PRODUCTS:
            return await interaction.response.send_message("❌ Unbekanntes Produkt.", ephemeral=True)
        import datetime
        ends_dt = discord.utils.utcnow() + datetime.timedelta(minutes=minuten)
        ends_iso = ends_dt.strftime("%Y-%m-%d %H:%M:%S")
        db.set_flash_sale(produkt, rabatt, ends_iso)
        p = PRODUCTS[produkt]
        embed = EmbedHelper.create_prestige_embed(
            title="🔥 BLITZANGEBOT / FLASH SALE 🔥",
            description=f"> {p['emoji']} **{p['name']}** ist ab sofort um **{rabatt}% reduziert**!\n> ⏳ Das Angebot endet in **{minuten} Minuten**.\n\nSichere dir dein Exemplar jetzt auf der Website oder erstelle ein Kauf-Ticket!",
            color=0xa33c3c, author_user=interaction.user, bot_user=self.bot.user
        )
        await interaction.response.send_message(embed=embed)
        info_ch = discord.utils.get(interaction.guild.text_channels, name="📢│── 𝗩𝗢𝗜𝗗 • 𝗜𝗡𝗙𝗢 ──") or interaction.channel
        if info_ch != interaction.channel:
            try: await info_ch.send(embed=embed)
            except Exception: pass

    @app_commands.command(name="partnercode", description="🤝 Erstelle deinen eigenen 10% Affiliate-Code & verdiene 15% Provision")
    @app_commands.choices(aktion=[app_commands.Choice(name="create", value="create")])
    @app_commands.guild_only()
    async def partnercode_command(self, interaction: discord.Interaction, aktion: app_commands.Choice[str], code: str):
        code_cl = code.upper().strip()
        if not code_cl.isalnum() or len(code_cl) < 3 or len(code_cl) > 16:
            return await interaction.response.send_message("❌ Code muss 3-16 Zeichen lang und alphanumerisch sein.", ephemeral=True)
        if db.affiliate_get(code_cl) or db.coupon_get(code_cl):
            return await interaction.response.send_message("❌ Dieser Code ist bereits vergeben.", ephemeral=True)
        db.affiliate_create(code_cl, interaction.user.id, interaction.user.name)
        embed = EmbedHelper.create_prestige_embed(
            title="🤝 Affiliate Code Erstellt",
            description=f"> Dein Partnercode lautet: **`{code_cl}`**\n> 🎁 Freunde sparen: **10% Rabatt**\n> 💰 Du verdienst: **15% Provision** als Coins\n\nTeile diesen Code auf TikTok/YouTube, um automatisch passives Einkommen zu generieren!",
