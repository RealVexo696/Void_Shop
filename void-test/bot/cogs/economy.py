...
class FFSelect(discord.ui.Select):
    def __init__(self, is_vip):
        ...
        super().__init__(placeholder="Wähle deine PC-Spezifikationen aus...", min_values=1, max_values=1, options=options)
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        emb = EmbedHelper.create_prestige_embed(
            ...
            description=f"> **Profil:** `{choice}`\n> **VIP Ultra Boost:** `{'Aktiviert 💎' if self.is_vip else 'Inaktiv (Benötigt VIP/Premium)'}`\n\n**Anleitung:**\nErsetze deine alte `ClientAppSettings.json` im Roblox Client-Ordner mit dieser maßgeschneiderten Datei!",
            color=0x39FF14, author_user=interaction.user, bot_user=interaction.client.user
        )
...
class EconomyCog(commands.Cog, name="EconomyCog"):
    ...
    async def auto_gamepass_scanner_task(self):
        """Scannt verifizierte User automatisch auf Gamepass-Besitz."""
                        ...
                        if member:
                            ...
                            if live_ch:
                                fomo_em = EmbedHelper.create_prestige_embed(
                                    ...
                                    description=f"***„🎉 Unser Auto-Gamepass Scanner hat soeben erkannt, dass {member.mention} den Gamepass im Roblox-Store erworben hat!***\n\n> 👑 **Rolle freigeschaltet:** `💎 Premium Buyer`\n> ⚡ **Kein Befehl nötig:** *Vollautomatische API Synchronisation!*",
                                    color=0x00FFFF
                                )
    ...
    async def copycat_and_bestseller_task(self):
        """Überwacht den Roblox-Katalog und postet den Bestseller der Woche."""
        if not self.bot.is_ready(): return
        for guild in self.bot.guilds:
            ...
            if staff_ch:
                warn_em = EmbedHelper.create_prestige_embed(
                    ...
                    description="> ⚠️ **Automatischer Schutz-Scan:** Es wurde ein potenzieller T-Shirt Klon im Roblox Katalog entdeckt (Asset-Match `98%`).\n> **Status:** DMCA Takedown Notice wurde vorbereitet.",
                    color=0xFF003C
                )
            ...
            if prod_ch:
                best_em = EmbedHelper.create_prestige_embed(
                    ...
                    description="> 🏆 **Platz 1:** `Premium FastFlags v2 Ultra Config`\n> 📦 **Verkäufe diese Woche:** `42x verkauft`\n\nSichere dir den Bestseller jetzt in `#🎟️│create-ticket`!",
                    color=0xFFD700
                )
    ...
    async def verify_command(self, interaction: discord.Interaction, roblox_username: str):
        ...
        # Speichere User für den Auto-Gamepass Scanner
    ...
    # ==========================================================================
    # /preview SLASH COMMAND (ROBLOX ASSET SCRAPER + WATERMARK)
    # ==========================================================================
    ...
    # ==========================================================================
    # /ffbuilder SLASH COMMAND (INTERAKTIVER FASTFLAG KONFIGURATOR)
    # ==========================================================================
    ...
    async def ffbuilder_command(self, interaction: discord.Interaction):
        ...
        emb = EmbedHelper.create_prestige_embed(
            ...
            description="> Wähle unten im Menü deine genauen PC-Spezifikationen und deinen Fokus aus.\n> Der Bot generiert in Echtzeit eine perfekt auf dein System abgestimmte `ClientAppSettings.json`!",
            color=0x00F0FF, author_user=interaction.user, bot_user=self.bot.user
        )
        await interaction.response.send_message(embed=emb, view=FFBuilderView(is_vip), ephemeral=True)

    # ==========================================================================
    # /mysterybox SLASH COMMAND (GACHA SYSTEM)
    # ==========================================================================
    ...
    async def mysterybox_command(self, interaction: discord.Interaction):
        ...
        box_em = EmbedHelper.create_prestige_embed(
            ...
            description="> 🎲 Simuliere Gacha-Rolle...\n> *Kosten: 100 Void-Coins bezahlt.*",
            color=0xFFD700, author_user=interaction.user, bot_user=self.bot.user
        )
        ...
        if roll < 0.01:
            ...
            win_em = EmbedHelper.create_prestige_embed(
                ...
                description=f"***„🎉 UNGLAUBLICH!!! {member.mention} hat den legendären Hauptgewinn aus der Mystery Box gezogen!***\n\n> 🎁 **Gewinn:** `🌟 VIP Rolle` + `500 Void-Coins`\n> 🪙 **Neuer Kontostand:** `{db.get_coins(member.id)} Coins`",
                color=0xFF003C, author_user=member, bot_user=self.bot.user
            )
            ...
            win_em = EmbedHelper.create_prestige_embed(
                ...
                description=f"> Herzlichen Glückwunsch! Du hast eine geheime **Premium FastFlag Ultra Config** gezogen!\n> ⚡ Die Konfigurationsdatei wurde angehängt.",
                color=0x00F0FF, author_user=member, bot_user=self.bot.user
            )
            await interaction.edit_original_response(embed=win_em, attachments=[dfile])
        elif roll < 0.30:
            win_em = EmbedHelper.create_prestige_embed(
                title="👕 T-SHIRT TEMPLATE VORLAGE (20% CHANCE)",
                description=f"> Sauber! Du hast ein zufälliges T-Shirt Template gewonnen!\n> Download: `https://void-shop.cloud/assets/mystery_shirt.png`",
                color=0x39FF14, author_user=member, bot_user=self.bot.user
            )
            await interaction.edit_original_response(embed=win_em)
        else:
            ...
            win_em = EmbedHelper.create_prestige_embed(
                ...
                description=f"> Du hast `+{bonus} Void-Coins` als Treuebonus zurückerhalten!\n> 🪙 **Neuer Kontostand:** `{db.get_coins(member.id)} Coins`",
                color=0xA0AEC0, author_user=member, bot_user=self.bot.user
            )
            await interaction.edit_original_response(embed=win_em)

    # ==========================================================================
    # /analytics SLASH COMMAND (MATPLOTLIB LIVE CHART)
    # ==========================================================================
    ...
    async def analytics_command(self, interaction: discord.Interaction):
        ...
        emb = EmbedHelper.create_prestige_embed(
            ...
            description="> 📊 **Aktuelle Auswertung:** Hier ist dein in Echtzeit gerendertes Wachstums- und Finanzdiagramm.\n> *Mitglieder und Robux-Umsatz verzeichnen einen massiven exponentiellen Anstieg!*",
            color=0xFFD700, author_user=interaction.user, bot_user=self.bot.user
        )
    ...
    # ==========================================================================
    # /vippass SLASH COMMAND (APPLE WALLET VIP PASS + QR CODE)
    # ==========================================================================
    ...
    async def vippass_command(self, interaction: discord.Interaction):
        ...
        emb = EmbedHelper.create_prestige_embed(
            ...
            description=f"> **Seriennummer:** `{serial}`\n> **Inhaber:** {member.mention}\n\nDeine hochauflösende Apple Wallet VIP-Mitgliedskarte wurde erfolgreich im Glanzdesign gerendert!",
            color=0xFFD700, author_user=member, bot_user=self.bot.user
        )
        emb.set_image(url=f"attachment://vippass_{member.id}.png")
        await interaction.followup.send(embed=emb, file=dfile)

    # ==========================================================================
    # /tryon SLASH COMMAND (3D ROBLOX FITTING ROOM)
    # ==========================================================================
