"""
Tickets Cog - Ticket-System mit Dropdown (Select Menu), Close-Process mit Web-Transkripten, Vouch-Leveling.
Ausnahmslos ALLE Embeds im App-Karten UI Design (0x2b2d31) mit Trennlinien, Blockquotes (>) und Button-Footer.
"""
...
class PurchaseQuestion2View(discord.ui.View):
    def __init__(self):
        ...
        self.add_item(discord.ui.Button(label="Powered by BotForge", style=discord.ButtonStyle.secondary, disabled=True, row=4, custom_id="footer_pq2"))
...
class CloseTicketMenu(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=None)
        self.add_item(discord.ui.Button(label="Powered by BotForge", style=discord.ButtonStyle.secondary, disabled=True, row=4, custom_id="footer_ctm"))
...
class CloseTicketView(discord.ui.View):
    """View für die Ticket-Steuerung im Ticket-Kanal (Hier bleiben BUTTONS wie gewünscht!)."""
    def __init__(self):
        super().__init__(timeout=None)
        self.add_item(discord.ui.Button(label="Powered by BotForge", style=discord.ButtonStyle.secondary, disabled=True, row=4, custom_id="footer_ctv"))
...
# --- TICKET ERSTELLEN SELECT MENU (Dropdown im Embed wie gewünscht!) ---
...
class TicketSelect(discord.ui.Select):
    def __init__(self):
        options = [
            discord.SelectOption(
                ...
                description="Erstellen um ein Produkt zu kaufen",
                emoji="🛒",
                value="buy"
            ),
            discord.SelectOption(
                label="Allgemeiner Support",
                description="Erstellen um technische Hilfe zu erhalten",
                emoji="⚙️",
                value="support"
            ),
            discord.SelectOption(
                label="Partnerschaft",
                description="Erstellen für Kooperationen",
                emoji="🤝",
                value="partner"
            )
        ]
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if choice == "buy":
            await self.view.create_custom_ticket(interaction, "🛒│kauf", "Kauf-Anfrage")
        elif choice == "support":
            await self.view.create_custom_ticket(interaction, "⚙️│support", "Allgemeiner Support")
        elif choice == "partner":
            await self.view.create_custom_ticket(interaction, "🤝│partner", "Partnerschafts-Anfrage")
...
class TicketButton(discord.ui.LayoutView):
    """Components-V2 Ticket-Panel: Container (type 17) mit Text + Select-Menü.
    Persistent (timeout=None), damit Buttons/Selects nach Neustart funktionieren."""

    def __init__(self):
        super().__init__(timeout=None)
        self.select = TicketSelect()
        # Callback an die View-Instanz binden (LayoutView hat selbst keine create_custom_ticket
        # über self.view automatisch — TicketSelect.callback nutzt self.view, das passt)
