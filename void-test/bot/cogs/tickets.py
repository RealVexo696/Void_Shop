...
async def warn_low_stock(guild, product_key):
    """Warnt im Ticket-Log, wenn ein Produkt unter 3 Keys fällt."""
    try:
        ...
        if stock < 3:
            ...
            msg = (f"{p.get('emoji','📦')} **{p.get('name', product_key)}** hat nur noch **{stock} Keys**.\n"
            ...
            if guild and guild.owner:
                try:
                    await guild.owner.send(f"⚠️ VOID Stock-Warnung: {p.get('name', product_key)} hat nur noch {stock} Keys.")
                except Exception:
                    pass
    except Exception:
        pass
...
class CouponModal(discord.ui.Modal, title="🏷️ Coupon einlösen"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not pct:
            return await interaction.response.send_message("❌ Ungültiger Coupon.", ephemeral=True)
        db.cart_set_coupon(self.channel_id, code)
        await interaction.response.send_message(f"✅ Coupon `{code}` angewendet. Bitte aktualisiere den Warenkorb mit einem Button-Klick oder Checkout.", ephemeral=True)
