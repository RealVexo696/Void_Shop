...
SUPPORT_CATEGORY = "🎟️│── 𝗩𝗢𝗜𝗗 • ＳＵＰＰＯＲＴ ──"
...
def product_catalog_text() -> str:
    """Schöner Markdown-Block mit allen Produkten + Preisen + Live-Stock."""
    ...
    for key in PRODUCT_ORDER:
        ...
        if db.is_unlimited(key):
            ...
            amount = db.stock_count(key)
            badge = "🟢" if amount > 5 else ("🟡" if amount > 0 else "🔴")
            stock = f"{badge} {amount} auf Lager"
        lines.append(f"{p['emoji']} **{p['name']}** — `{p['price']}` · **{stock}**\n-# {p['desc']}")
    return "\n".join(lines)


def stock_signature() -> tuple:
    """Kompakte Signatur, um Stock-Änderungen alle 5 Sekunden zu erkennen."""
    return tuple((k, db.is_unlimited(k), db.stock_count(k)) for k in PRODUCT_ORDER)


def build_products_panel_view(ticket_mention="#create-ticket"):
    """Aktuelles Produktpanel für #products, editierbar vom Auto-Refresh."""
    return build_layout(
        title="📦 𝗩𝗢𝗜𝗗 • Produkt-Katalog & Live-Stock",
        body=(
            "Alle Produkte, Preise und Lagerbestände werden automatisch aktuell gehalten.\n\n"
            + product_catalog_text()
            + f"\n\n🛒 **Kaufen:** Öffne ein Ticket in {ticket_mention} und wähle dein Produkt per Klick.\n"
              "-# Stock-Auto-Refresh aktiv · Unlimited-Produkte brauchen keinen Lizenz-Key."
        ),
        accent=ACCENT_BUY,
        footer=True,
        timeout=None,
...
async def execute_ticket_close_process(channel, closed_by_user, bot_user):
    ...
    try:
        stats_cog = bot_user._state._get_client().get_cog("StatsCog") if bot_user else None
        if stats_cog and guild:
...
# ==================================================================
class FeedbackModal(Modal, title="⭐ Deine VOID Bewertung"):
    ...
    feedback = TextInput(
        label="Deine Erfahrung mit Produkt & Support",
        style=discord.TextStyle.paragraph,
        placeholder="Beispiel: Schnelle Lieferung, netter Support, Produkt funktioniert perfekt...",
        required=True,
        max_length=1000,
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        stars_full = "⭐" * self.stars
        stars_empty = "☆" * (5 - self.stars)
        stars_str = stars_full + stars_empty
        vouch_count = db.add_user_vouch(member.id)
        db.add_supporter_review(member.name, self.stars)

        reward = ""
        reward_label = "Noch kein neuer Rang — sammle weiter Vouches."
        try:
            ...
            if vouch_count == 1 and r_bronze:
                ...
                reward = "\n🎁 **Belohnung:** `🥉 Bronze Buyer` + `20 Void-Coins`"
                reward_label = "🥉 Bronze Buyer + 20 Void-Coins"
            elif vouch_count == 3 and r_silver:
                await member.add_roles(r_silver)
                reward = "\n🎁 **Belohnung:** `🥈 Silver Buyer` + gratis T-Shirt Vorlage"
                reward_label = "🥈 Silver Buyer + gratis Vorlage"
            elif vouch_count == 5 and r_gold:
                await member.add_roles(r_gold)
                reward = "\n🎁 **Belohnung:** `🥇 Gold Buyer` + VIP-Lounge"
                reward_label = "🥇 Gold Buyer + VIP-Lounge"
            elif vouch_count >= 10 and r_diamond:
                await member.add_roles(r_diamond)
                reward = "\n💎 **Belohnung:** `💎 Diamond Buyer` + 10% Lifetime-Rabatt"
                reward_label = "💎 Diamond Buyer + 10% Lifetime-Rabatt"
        except Exception:
            pass
        ...
        if vouch_ch:
            accent = 0xFFD700 if self.stars >= 5 else (0x00D26A if self.stars >= 4 else 0x5865F2)
            body = (
                "### ✅ Verifizierte Kundenbewertung\n"
                f"**Kunde:** {member.mention}\n"
                f"**Produkt:** `{self.product_name}`\n"
                f"**Rating:** {stars_str}  `({self.stars}/5)`\n"
                f"**Vouch-Level:** `{vouch_count}x Vouch`\n"
                f"**Reward:** {reward_label}\n\n"
                "### 💬 Rezension\n"
                f"> {str(self.feedback.value).replace(chr(10), chr(10) + '> ')}\n\n"
                "-# Danke für dein Vertrauen in 𝗩𝗢𝗜𝗗ﾒ𝗦𝗛𝗢𝗣 — echte Bewertungen helfen neuen Kunden."
            )
            row = discord.ui.ActionRow()
            row.add_item(discord.ui.Button(label=f"{self.stars}/5 Sterne", emoji="⭐", style=discord.ButtonStyle.secondary, disabled=True))
            row.add_item(discord.ui.Button(label=f"{vouch_count}x Vouch", emoji="🤝", style=discord.ButtonStyle.secondary, disabled=True))
            cont = PrestigeContainer(
                "🌟 𝗩𝗢𝗜𝗗 • Neue Premium-Bewertung",
                body,
                accent=accent,
                author=member,
                items=[row],
        ...
        thanks = build_layout(
            ...
            f"Danke für dein Feedback, {member.mention}!\n"
            f"Deine Bewertung wurde in {vouch_ch.mention if vouch_ch else '#vouches'} veröffentlicht.{reward}\n\n"
            "Das Ticket wird jetzt sauber geschlossen und archiviert.",
            accent=ACCENT_BUY,
            author=member,
        )
...
async def deliver_cart(interaction, channel_id, buyer_id, cart):
    ...
    if not delivered:
        return await interaction.response.send_message(
            view=build_layout(
                "❌ Bestellung konnte nicht geliefert werden",
                "Alle Produkte im Warenkorb sind aktuell ohne Key/Stock.\n"
                "Bitte nutze `/addkeys` oder setze ein Produkt mit `/un` auf Unlimited und versuche es erneut.",
                accent=0xED4245, timeout=None
            ),
            ephemeral=True
        )

    missing_txt = f"\n⚠️ Ausverkauft (nicht geliefert): {', '.join(missing)}" if missing else ""
    dm_status = "✅ Lieferung per DM zugestellt" if dm_ok else "⚠️ DM fehlgeschlagen — Details im Ticket"
    cont = PrestigeContainer(
...
class ProductChoiceView(discord.ui.LayoutView):
    ...
    def __init__(self):
        ...
        for idx, key in enumerate(PRODUCT_ORDER):
            ...
            out_of_stock = (not unlimited and stock <= 0)
            label = f"{p['name']}"
            b = discord.ui.Button(
                label=label,
                emoji=p["emoji"],
                style=discord.ButtonStyle.success if not out_of_stock else discord.ButtonStyle.secondary,
                disabled=False,  # Tickets dürfen auch bei 0 Stock geöffnet werden; Team kann /addkeys oder /un nutzen.
            )
...
def ticket_panel_body() -> str:
    ...
    return (
        ...
        "Wähle unten dein Anliegen. Der Bot erstellt danach automatisch einen privaten Kanal für dich.\n\n"
        "### 🎫 Ticket-Arten\n"
        "🛒 **Produkt kaufen** — Produkt per Button wählen, Warenkorb nutzen, Mengenrabatt erhalten.\n"
        "⚙️ **Fragen / Support** — Hilfe bei Setup, Zahlung, Downloads, Keys oder Produkten.\n"
        "🤝 **Partnerschaft** — Kooperation, Werbung, Partner-Server oder langfristige Zusammenarbeit.\n\n"
        + product_catalog_text() +
        "\n\n### ⚡ Hinweise\n"
        "• **2 Artikel:** −10% · **3 Artikel:** −15% · **4+ Artikel:** −20%\n"
        "• 🟣 **Unlimited** bedeutet: kein Key nötig, unbegrenzt lieferbar.\n"
        "• Normale Produkte werden nach Zahlung automatisch per DM geliefert.\n"
        "-# Live-Stock wird automatisch aktualisiert."
    )
...
class TicketButton(discord.ui.LayoutView):
    ...
    async def create_custom_ticket(self, interaction, prefix, ticket_type, product_key=None):
        ...
        try:
            ...
            await _reply(f"✅ Dein Ticket wurde erstellt: {channel.mention} — unser Team meldet sich gleich bei dir.")
...
class TicketsCog(commands.Cog, name="TicketsCog"):
    def __init__(self, bot):
        self.bot = bot
        self._last_stock_signature = None
        self.stock_panel_refresh_task.start()

    async def cog_unload(self):
        self.stock_panel_refresh_task.cancel()

    @tasks.loop(seconds=5)
    async def stock_panel_refresh_task(self):
        """Aktualisiert Ticket-Erstell-Panel und Produkt-Katalog, sobald Stock/Unlimited sich ändert."""
        if not self.bot.is_ready():
            return
        sig = stock_signature()
        if sig == self._last_stock_signature:
            return
        self._last_stock_signature = sig
        for guild in self.bot.guilds:
            try:
                ticket_ch = discord.utils.get(guild.text_channels, name="🎟️│create-ticket")
                products_ch = discord.utils.get(guild.text_channels, name="📦│products")
                ticket_mention = ticket_ch.mention if ticket_ch else "#create-ticket"

                if ticket_ch:
                    async for msg in ticket_ch.history(limit=20):
                ...
                if products_ch:
                    async for msg in products_ch.history(limit=20):
                        if msg.author == self.bot.user:
                            try:
                                await msg.edit(view=build_products_panel_view(ticket_mention))
                                break
                            except Exception:
                                pass
            except Exception as e:
                logger.debug(f"Stock-Panel Refresh Fehler in {guild.name}: {e}")

    @stock_panel_refresh_task.before_loop
    async def before_stock_panel_refresh_task(self):
