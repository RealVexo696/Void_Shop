...
Alle interaktiven Embeds im App-Karten UI Design (0x2b2d31).
"""
            ...
            if vouch_count == 1:
                ...
                try:
                    dm_em = EmbedHelper.create_prestige_embed(
                        ...
                        description="Vielen Dank für deinen 3. Vouch! Hier ist dein exklusives T-Shirt Template:\n> Download: `https://void-shop.cloud/assets/free_template.png`",
                        color=0xC0C0C0, bot_user=interaction.client.user
                    )
...
class CloseTicketMenu(discord.ui.View):
    ...
    @discord.ui.button(label="🔒 Direkt Schließen", style=discord.ButtonStyle.danger, custom_id="ctm_close")
    async def direct_close(self, interaction: discord.Interaction, button: discord.ui.Button):
        for child in self.children: child.disabled = True
        await interaction.response.edit_message(view=self)
        await execute_ticket_close_process(interaction.channel, interaction.user, interaction.client.user)

    @discord.ui.button(label="🛒 Kauf bewerten & Belohnung erhalten", style=discord.ButtonStyle.success, custom_id="ctm_review")
    async def review_reward(self, interaction: discord.Interaction, button: discord.ui.Button):
        for child in self.children: child.disabled = True
        await interaction.response.edit_message(view=self)
        
        # APP-CARD LOOK (0x2b2d31) FÜR FRAGE 2 VIEW MIT BUTTONS
    ...
    @discord.ui.button(label="❌ Abbrechen", style=discord.ButtonStyle.secondary, custom_id="ctm_cancel")
...
class CloseTicketView(discord.ui.View):
    ...
    async def close_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        # APP-CARD LOOK (0x2b2d31) FÜR SCHLIESSEN MENÜ MIT BUTTONS
...
class TicketButton(discord.ui.View):
    ...
    async def create_custom_ticket(self, interaction: discord.Interaction, prefix: str, ticket_type: str):
        ...
        try:
            ...
            # APP-CARD LOOK (0x2b2d31) FÜR NEUES TICKET MIT BUTTONS
            embed = EmbedHelper.create_prestige_embed(
