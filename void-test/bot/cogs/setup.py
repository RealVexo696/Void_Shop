...
class SetupCog(commands.Cog, name="SetupCog"):
    ...
    async def update_permissions_command(self, ctx):
        """
        Setzt blitzschnell und hochpräzise die perfekten Kanalrechte für alle 23 Rollen
        auf sämtlichen existierenden Kanälen (ohne Rollen neu zu erstellen).
        """
        guild = ctx.guild
        bot_member = guild.me
    ...
    async def create_roles_command(self, ctx):
        """
        Erstellt alle 23 Premium-Rollen mit sofortigem Rechte-Check, Rollenlimit-Check
        und zuverlässigem Rate-Limit-Schutz (2.0s Pause).
        """
        guild = ctx.guild
        bot_member = guild.me
        ...
        # 1. SOFORT-CHECK: Hat der Bot überhaupt das Recht 'Rollen verwalten'?
        ...
        # 2. SOFORT-CHECK: Ist das Discord Rollenlimit (250) fast erreicht?
