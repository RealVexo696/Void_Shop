...
Rollenlimit-Prüfung und App-Karten UI Design (0x2b2d31) für alle interaktiven Embeds.
"""
...
class SetupCog(commands.Cog, name="SetupCog"):
    ...
    async def update_permissions_command(self, ctx):
        ...
        if not bot_member.guild_permissions.manage_channels or not bot_member.guild_permissions.manage_roles:
            embed_warning = create_prestige_embed(
                ...
                description="> Mir fehlt die Berechtigung **'Kanäle verwalten'** oder **'Rollen verwalten'** auf diesem Server!",
                color=0xff003c,
        ...
        progress_embed = create_prestige_embed(
            ...
            description=f"> ⚙️ **Synchronisiere Rechte für alle 23 Rollen...**\n> Prüfe und bearbeite `{len(guild.channels)} Kanäle & Kategorien` in Echtzeit. Bitte warten...",
            color=0x00f0ff,
            author_user=ctx.author,
            bot_user=self.bot.user
        )
        ...
        success_embed = create_prestige_embed(
            ...
            description=f"> 🔒 **Kanäle konfiguriert:** `{channels_processed} von {len(guild.channels)}`\n\n"
                        f"Sämtliche Kanäle und Kategorien besitzen nun 100% passgenaue Zugriffsrechte für alle 23 Rollen!",
            color=0x39ff14,
            author_user=ctx.author,
            bot_user=self.bot.user
        )
    ...
    async def create_roles_command(self, ctx):
        ...
        progress_embed = create_prestige_embed(
            ...
            description="> ⚙️ Initialisiere Systemprüfungen...\nBitte warten.",
            color=0x00f0ff,
            author_user=ctx.author,
            bot_user=self.bot.user
        )
        ...
        role_colors = {
            # --- STAFF ROLES ---
            "👑│ 𝗩𝗢𝗜𝗗 • Owner": (0xff003c, True),        # Crimson Red
            "👑│ 𝗩𝗢𝗜𝗗 • Co-Owner": (0xff3366, True),     # Light Pink-Red
            "🛠️│ 𝗩𝗢𝗜𝗗 • Admin": (0x00f0ff, True),        # Neon Cyan
            "⚙️│ 𝗩𝗢𝗜𝗗 • Manager": (0x00a8a8, True),      # Dark Teal
            "🛡️│ 𝗩𝗢𝗜𝗗 • Moderator": (0x39ff14, False),    # Neon Green
            "🎫│ 𝗩𝗢𝗜𝗗 • Support": (0x20b2aa, False),      # Light Sea Green
            "🚨│ 𝗩𝗢𝗜𝗗 • Trial Mod": (0xadff2f, False),    # Green Yellow
            
            # --- SPECIAL ROLES ---
            "🤝│ 𝗩𝗢𝗜𝗗 • Partner": (0xffa500, False),      # Orange
            "💎│ 𝗩𝗢𝗜𝗗 • Booster": (0xf47fff, False),      # Pink
            "🌟│ 𝗩𝗢𝗜𝗗 • VIP": (0xffd700, False),          # Gold (VIP)
            "🫂│ 𝗩𝗢𝗜𝗗 • Friend": (0xff69b4, False),        # Hot Pink
            "👤│ 𝗩𝗢𝗜𝗗 • Verified": (0x7f8c8d, False),      # Gray-Blue (Verified)
            
            # --- CUSTOMER ROLES ---
            "🛒│ 𝗩𝗢𝗜𝗗 • Customer": (0xffff00, False),     # Yellow
            "💎│ 𝗩𝗢𝗜𝗗 • Premium Buyer": (0x00ffff, False), # Cyan Customer
            
            # --- REWARD ROLES ---
            "🥉│ 𝗩𝗢𝗜𝗗 • Bronze Buyer": (0xcd7f32, False),       # Bronze
            "🥈│ 𝗩𝗢𝗜𝗗 • Silver Buyer": (0xc0c0c0, False),       # Silver
            "🥇│ 𝗩𝗢𝗜𝗗 • Gold Buyer": (0xffd700, False),         # Gold
            "💎│ 𝗩𝗢𝗜𝗗 • Diamond Buyer": (0xb9f2ff, False),       # Diamond
            
            # --- NOTIFICATION ROLES ---
            "📢│ 𝗩𝗢𝗜𝗗 • Announcement Ping": (0x7289da, False),  # Discord Blue
            "🎁│ 𝗩𝗢𝗜𝗗 • Giveaway Ping": (0xff4500, False),      # Red-Orange
            "📦│ 𝗩𝗢𝗜𝗗 • Product Ping": (0x32cd32, False),       # Lime Green
            
            # --- BASE ROLES ---
            "👥│ 𝗩𝗢𝗜𝗗 • Member": (0xa0a0a0, False),       # Light Gray
            "🤖│ 𝗩𝗢𝗜𝗗 • Bot": (0x4a00a8, False)           # Dark Purple
        }
        ...
        for idx, (role_name, (color_hex, is_admin)) in enumerate(role_colors.items(), 1):
            progress_embed.description = (
                ...
                f"> Aktuell: `{role_name}`\n\n"
                f"> ⚠️ *Hinweis: Discord hat ein strenges Rate-Limit für Rollen. Sollte der Bot hier kurz pausieren, bricht er NICHT ab, sondern wartet lediglich das Discord Rate-Limit ab! Bitte einfach laufen lassen.*"
            )
            try:
                ...
                if not new_role:
                    embed_fail = create_prestige_embed(
                        ...
                        description=(
                            ...
                            "**Bitte ziehe die Bot-Rolle in den Einstellungen ganz nach oben und versuche es in wenigen Minuten erneut!**"
                        ),
                        color=0xff003c,
                        author_user=ctx.author,
                        bot_user=self.bot.user
    ...
    async def start(self, ctx):
        # APP-CARD LOOK (0x2b2d31) FÜR SETUP-WAHL EMBED MIT BUTTONS
        embed_choice = create_prestige_embed(
            ...
            description=(
                ...
                "> 💬 │ **27 Textkanäle, 7 Voicekanäle & 7 professionelle Log-Kanäle (Gesamt: 41 Kanäle)**\n"
                "> ⚙️ │ **Vollständiges Embed-Design in allen Infokanälen**\n\n"
                "~~                                                              ~~\n\n"
                "Bitte wähle eine Option aus:\n\n"
                "🧹 **Komplett neu aufsetzen:** Löscht alle Kanäle (außer den aktuellen) und baut neu auf.\n"
                "➕ **Nur hinzufügen:** Ergänzt das Layout parallel.\n\n"
                "~~                                                              ~~"
            ),
            color=0x2b2d31,  # APP-CARD DARK MODE LOOK
            author_user=ctx.author,
            bot_user=self.bot.user
        )
        ...
        # D) TICKET PANEL (App-Card Look 0x2b2d31 mit Buttons)
        ...
        if c_ticket_panel:
            ...
            embed_ticket_panel = create_prestige_embed(
                title="🎟️ 𝗩𝗢𝗜𝗗 • Support & Kauf-Center",
                description=(
        ...
        # G) VERIFY HERE PANEL (App-Card Look 0x2b2d31 mit Buttons)
        ...
        if c_v_here:
            ...
            embed_v_here = create_prestige_embed(
                title="🔐 𝗩𝗢𝗜𝗗 • SERVEREINTRETEN VERIFIZIERUNG",
                description=(
