...
class SetupConfirmationView(discord.ui.View):
    """Vertikal gestaffeltes App-Karten Menü (row=0, row=1, row=2, row=3)."""
    def __init__(self, ctx):
        super().__init__(timeout=60)
        self.ctx = ctx
        self.value = None
        self.add_item(discord.ui.Button(label="Powered by BotForge", style=discord.ButtonStyle.secondary, disabled=True, row=3, custom_id="footer_setup_confirm"))
    ...
    @discord.ui.button(label="➕ Nur hinzufügen", style=discord.ButtonStyle.success, emoji="➕", row=1)
    ...
    @discord.ui.button(label="❌ Abbrechen", style=discord.ButtonStyle.secondary, emoji="❌", row=2)
    ...
    async def start(self, ctx):
        embed_choice = create_prestige_embed(
            ...
            description=(
                ...
                "> 💬 │ **27 Textkanäle, 7 Voicekanäle & 7 Log-Kanäle (Gesamt: 41 Kanäle)**\n"
                "> ⚙️ │ **Vollständiges High-End Embed-Design in allen Infokanälen**\n"
                "~~                                                              ~~\n"
