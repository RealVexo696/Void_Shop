...
class SetupCog(commands.Cog, name="SetupCog"):
    ...
    async def start(self, ctx):
        setup_body = (
            ...
            "💬 │ **30 Textkanäle, 7 Voicekanäle & 7 Log-Kanäle (Gesamt: 44 Kanäle)**\n"
        ...
        product_showcases = [
            ...
            ("💉│fflags-injector", "💉 FFlags Injector", 0x00d26a,
             ...
             "🎯 1-Klick Setup, keine manuelle Arbeit\n\n"
             "💰 **Preis:** `300 R$ / 3,00 €`"),
            ("🛡️│anti-ban", "🛡️ Anti-Ban", 0xff4757,
             ...
             "🔄 Laufende Updates gegen neue Detections\n\n"
             "💰 **Preis:** `450 R$ / 4,50 €`"),
        ]
        ...
        # C) PRODUCTS — Components V2 mit zentralem Katalog (inkl. neuer Produkte)
        c_products = channels_by_name.get("📦│products")
        if c_products:
            from bot.cogs.tickets import product_catalog_text
            ticket_mention = c_ticket_mention
            products_view = build_layout(
                title="📦 𝗩𝗢𝗜𝗗 • ＵＮＳＥＲＥ ＰＲＯＤＵＫＴＥ",
                body=(
                    product_catalog_text()
                    + f"\n\n🛒 **Kaufen?** Erstelle ein Ticket in {ticket_mention} "
                    "und wähle dein Produkt im Menü aus!"
                ),
                accent=0x00d26a,
            )
            await c_products.send(view=products_view)
