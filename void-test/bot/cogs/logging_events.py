...
class LoggingEventsCog(commands.Cog, name="LoggingEventsCog"):
    ...
    async def on_voice_state_update(self, member, before, after):
        ...
        if before.channel is None and after.channel is not None:
            if any(x in after.channel.name.lower() for x in ["talk", "wartezimmer", "support", "voice"]):
                try:
                    from bot.cogs.components_v2 import PrestigeContainer
                    v = discord.ui.LayoutView(timeout=1800)
                    btn = discord.ui.Button(label="🙋‍♂️ Supporter herbeirufen", style=discord.ButtonStyle.primary, custom_id=f"callsupp:{member.id}:{after.channel.id}")
                    v.add_item(PrestigeContainer("🎙️ Push-to-Talk Live-Support", f"👋 {member.mention} wartet in {after.channel.mention}.\nBrauchst du persönliche Hilfe im Voice? Klicke hier:", accent=0x5865f2, items=[btn]))
                    talk_ch = discord.utils.get(member.guild.text_channels, name="🎙️│── 𝗩𝗢𝗜𝗗 • ＴＡＬＫ ──") or discord.utils.get(member.guild.text_channels, name="💬│── 𝗩𝗢𝗜𝗗 • 𝗖𝗛𝗔𝗧 ──") or log_channel
                    await talk_ch.send(content=member.mention, view=v)
                except Exception:
                    pass
            embed = EmbedHelper.create_prestige_embed(
    ...
    async def on_interaction(self, interaction: discord.Interaction):
        if interaction.type == discord.InteractionType.component:
            ...
            if cid.startswith("cart_"):
                ...
                return
            if cid.startswith("paystatus:"):
                try:
                    status = cid.split(":", 1)[1]
                    from bot.cogs.tickets import is_staff
                    if not is_staff(interaction.user):
                        return await interaction.response.send_message("❌ Nur Team.", ephemeral=True)
                    db.set_payment_status(interaction.channel.id, status)
                    await interaction.response.send_message(f"✅ Payment-Status gesetzt: **{status}**")
                except Exception as e:
                    logger.error(f"Payment-Status Fehler: {e}")
                return
            if cid.startswith("callsupp:"):
                try:
                    parts = cid.split(":")
                    uid, vid = parts[1], parts[2]
                    staff = [r.mention for r in interaction.guild.roles if any(x in r.name for x in ["Admin", "Support", "Owner", "Manager", "Staff"])]
                    pings = " ".join(staff[:3]) or "@here"
                    await interaction.response.send_message(f"🚨 **LIVE SUPPORT RUF:** {pings}\nEin Kunde (<@{uid}>) benötigt sofortige Assistenz im Sprachkanal <#{vid}>!")
                except Exception:
                    pass
                return
