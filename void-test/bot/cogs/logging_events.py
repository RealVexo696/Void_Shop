...
Inklusive Vollautomatischem Void-Bounty Milestone-System (Auto-Delivery für Invites).
"""
...
class LoggingEventsCog(commands.Cog, name="LoggingEventsCog"):
    ...
    async def on_member_join(self, member):
        """Loggt Serverbeitritte, sendet Willkommens-Embeds & Invite-Tracking mit Auto-Delivery."""
        guild = member.guild
        ...
        if used_invite:
            ...
            # 🎁 VOID-BOUNTY MILESTONE SYSTEM (AUTO-DELIVERY)
            if total_invs in [5, 10, 20]:
                ...
                if gen_ch:
                    ...
                    m_desc = f"***„🎉 Herzlichen Glückwunsch an {used_invite.inviter.mention}! Du hast {total_invs} Freunde auf unseren Server eingeladen!***\n\n"
                    if total_invs == 5:
                    ...
                    bounty_em = EmbedHelper.create_prestige_embed(title=m_title, description=m_desc, color=0xFFD700)
                    bounty_em.set_thumbnail(url=used_invite.inviter.display_avatar.url)
                    await gen_ch.send(embed=bounty_em)

                # Send Auto-Delivery DM
                try:
                    ...
                    dm_em = EmbedHelper.create_prestige_embed(title=dm_title, description=dm_desc, color=0xFFD700, bot_user=self.bot.user)
