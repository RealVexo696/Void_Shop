...
DISCORD_INVITE_URL = "https://discord.gg/voidshop"
  ...
  <div class=\"nav-shell\">
    <nav class=\"nav wrap\">
      ...
      <div class=\"links\" aria-label=\"Navigation\">
        <a href=\"/#about\">Wer wir sind</a><a href=\"/#offer\">Leistungen</a><a href=\"/#products\">Produkte</a><a href=\"/changelog\">Changelog</a><a href=\"/partner\">Partner</a>
      </div>
...
def render_compare_rows(products):
    rows=[]
    for k,p in products.items():
        rows.append(f"<tr><td style='padding:12px;border-bottom:1px solid var(--line)'>{escape(p['name'])}</td><td style='padding:12px;border-bottom:1px solid var(--line)'>{escape(str(p['robux']))} R$</td><td style='padding:12px;border-bottom:1px solid var(--line)'>{escape(p.get('key_needed','Ja'))}</td><td style='padding:12px;border-bottom:1px solid var(--line)'>Auto / Ticket</td><td style='padding:12px;border-bottom:1px solid var(--line)'>{escape(PRODUCT_GROUP_LABELS.get(product_group(k),'Tools'))}</td></tr>")
    return "".join(rows)
...
def render_recent_sales(status):
    ...
    if not sales:
        return '<article class="change-item"><strong>Noch keine öffentlichen Verkäufe</strong><p>Sobald Verkäufe vorhanden sind, erscheinen sie hier anonymisiert.</p></article>'
    return "".join(f"<article class='change-item'><strong>{escape(x.get('product_name','Produkt'))}</strong><p>gekauft · {escape(x.get('time',''))}</p></article>" for x in sales)

def render_landing(products, status, best_key=None):
    bot_state = "Online" if status.get("bot_online") else "Offline"
    html = base_head() + nav() + f"""
  <main id=\"top\">
    ...
    <section class=\"wrap\"><div class=\"section-top\"><div><div class=\"eyebrow\">Warum VOID?</div><h2>Weniger Schritte, mehr Klarheit.</h2></div><p>Der Shop bleibt einfach — von der Auswahl bis zur Lieferung.</p></div><div class=\"proof-grid\"><div class=\"proof\"><strong>Keine unnötigen Schritte</strong><span>Produkt wählen und Ticket öffnen.</span></div><div class=\"proof\"><strong>Private Tickets</strong><span>Alles bleibt nachvollziehbar.</span></div><div class=\"proof\"><strong>Nachvollziehbare Lieferung</strong><span>Receipt, Bestellnummer und Delivery-Log.</span></div></div></section>
...
def render_store(products):
    cards = "".join(f"""
    <article class=\"product-card\" style=\"color:var(--bg)\"><div class=\"product-head\"><div class=\"mockup\">{escape(product_mockup(k))}</div><span class=\"tag\">{escape(PRODUCT_GROUP_LABELS.get(product_group(k),'Tools'))}</span></div><h3>{escape(p['emoji'])} {escape(p['name'])}</h3><p>{escape(p['desc'])}</p><div class=\"price\"><div><strong>{escape(str(p['robux']))} R$</strong><span>{escape(p['price'])}</span></div><span class=\"availability\">{escape(p.get('stock_label','auf Lager'))}</span></div><button class=\"product-action\" onclick=\"openModal('{escape(p['name'])}','discord')\">Kaufanfrage senden</button></article>""" for k, p in products.items())
  ...
  <script>let selectedProduct='',selectedMode='discord';function openModal(p,m){{selectedProduct=p;selectedMode=m;document.getElementById('modal-title').textContent=p;document.getElementById('buyModal').style.display='flex'}}function closeModal(){{document.getElementById('buyModal').style.display='none'}}async function confirmBuy(){{const user=document.getElementById('userIdentifier').value;if(!user){{alert('Bitte Discord Username/ID eingeben.');return}}try{{await fetch('/api/buy',{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify({{user:user,product:selectedProduct,mode:selectedMode}})}});alert('Kaufanfrage gesendet. Bitte Discord prüfen.');closeModal();}}catch(e){{alert('Fehler beim Senden.')}}}}</script>
""" + footer() + theme_script()
...
def render_admin():
  ...
  <script>
    async function loadAdmin(){
      ...
      document.getElementById('salesFilter').innerHTML='<option value="">Alle Produkte</option>'+p.products.map(x=>`<option value="${x.key}">${x.name}</option>`).join('');
      document.getElementById('adminProducts').innerHTML=p.products.map(x=>`<article class="offer"><h3>${x.emoji} ${x.name}</h3><p>Preis: <b>${x.robux} R$</b><br>Stock: <b>${x.stock_label}</b><br>Unlimited: <b>${x.unlimited?'Ja':'Nein'}</b><br>Aktiv: <b>${x.active?'Ja':'Nein'}</b></p><div class="hero-actions"><button class="btn btn-primary" onclick="toggleUn('${x.key}',${!x.unlimited})">Unlimited ${x.unlimited?'aus':'an'}</button><button class="btn btn-secondary" onclick="editProduct('${x.key}',${x.robux})">Bearbeiten</button><button class="btn btn-secondary" onclick="addKeys('${x.key}')">Keys add</button><button class="btn btn-secondary" onclick="showKeys('${x.key}')">Keys Liste</button></div></article>`).join('');
      const t=await fetch('/api/admin/tickets').then(r=>r.json());
      document.getElementById('ticketList').innerHTML=(t.tickets||[]).map(x=>`<article class="change-item"><strong>#${x.name}</strong><p>${x.topic||'Support'}</p></article>`).join('')||'<p class="muted">Keine offenen Tickets.</p>';
      loadSales();
    }
    ...
    async function showKeys(k){const d=await fetch('/api/admin/products/'+k+'/keys').then(r=>r.json());alert((d.keys||[]).map(x=>`${x.used?'USED':'FREE'} | ${x.key} | ${x.used_by||''}`).join('\n')||'Keine Keys');}
    async function loadSales(){const f=document.getElementById('salesFilter').value;const d=await fetch('/api/admin/sales'+(f?'?product='+encodeURIComponent(f):'')).then(r=>r.json());document.getElementById('salesList').innerHTML=d.sales.map(x=>`<article class="change-item"><strong>${x.order_id||''} · ${x.product_name} · ${x.robux} R$</strong><p>${x.buyer_name} · ${x.time}</p></article>`).join('')||'<p class="muted">Keine Verkäufe.</p>';}
    document.getElementById('salesFilter').addEventListener('change',loadSales);loadAdmin();setInterval(loadAdmin,20000);
  </script>
""" + footer() + theme_script()
