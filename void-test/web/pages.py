...
def render_admin():
  ...
  <script>
    ...
    async function loadAdmin(){
      ...
      document.getElementById('salesFilter').innerHTML='<option value="">Alle Produkte</option>'+p.products.map(x=>`<option value="${x.key}" ${x.key===oldProd?'selected':''}>${x.name}</option>`).join('');
