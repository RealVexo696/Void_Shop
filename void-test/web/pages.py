...
def theme_script():
  ...
  <script>
    (function(){
      ...
      }).catch(function(){});} loadStatus(); setInterval(loadStatus,30000);
    })();
  </script>
</body>
</html>"""
...
def render_product_cards(products, best_key=None):
    ...
    for key, p in products.items():
        ...
        cards.append(f"""
          <article class=\"product-card\" data-group=\"{escape(group)}\">
            <div class=\"product-head\"><div class=\"mockup\">{escape(product_mockup(key))}</div><span class=\"tag\">{escape(tag)}</span></div>
            <h3>{escape(p['name'])}</h3><p>{escape(p['desc'])}</p>
            <div class=\"price\"><div><strong>{escape(str(p['robux']))} R$</strong><span>{escape(p.get('euro', p['price'].split('/')[-1].strip()))}</span></div><span class=\"availability\">{escape(stock)}</span></div>
            <div class=\"product-actions\"><a class=\"product-action\" href=\"/product/{escape(key)}\">Details</a><a class=\"product-action secondary\" href=\"/store\">In Discord kaufen</a></div>
          </article>""")
    return "\n".join(cards)
...
def render_admin():
  ...
  <main class=\"wrap\"><section><div class=\"eyebrow\">Admin</div><h1>Dashboard.</h1><p class=\"lead\">Live-Statistiken, Produktbestand, Unlimited-Status und Sales.</p><div class=\"hero-actions\"><a class=\"btn btn-secondary\" href=\"/admin/logout\">Abmelden</a></div></section><section style=\"padding-top:0\"><div class=\"status-bar\" id=\"adminKpis\"></div></section><section style=\"padding-top:0\"><div class=\"section-top\"><div><div class=\"eyebrow\">Produkte</div><h2>Produktverwaltung.</h2></div><p>Unlimited setzen oder Keys hinzufügen.</p></div><div class=\"offer-grid\" id=\"adminProducts\"></div></section><section style=\"padding-top:0\"><div class=\"section-top\"><div><div class=\"eyebrow\">Sales</div><h2>Verkäufe filtern.</h2></div><p>Letzte Verkäufe nach Produkt ansehen.</p></div><select id=\"salesFilter\" class=\"btn btn-secondary\"><option value=\"\">Alle Produkte</option></select><div class=\"change-list\" id=\"salesList\" style=\"margin-top:16px\"></div></section></main>
  <script>
    async function loadAdmin(){
      ...
      document.getElementById('adminProducts').innerHTML=p.products.map(x=>`<article class="offer"><h3>${x.emoji} ${x.name}</h3><p>Stock: <b>${x.stock_label}</b><br>Unlimited: <b>${x.unlimited?'Ja':'Nein'}</b></p><div class="hero-actions"><button class="btn btn-primary" onclick="toggleUn('${x.key}',${!x.unlimited})">Unlimited ${x.unlimited?'aus':'an'}</button><button class="btn btn-secondary" onclick="addKeys('${x.key}')">Keys</button></div></article>`).join('');
      loadSales();
    }
    ...
    async function loadSales(){const f=document.getElementById('salesFilter').value;const d=await fetch('/api/admin/sales'+(f?'?product='+encodeURIComponent(f):'')).then(r=>r.json());document.getElementById('salesList').innerHTML=d.sales.map(x=>`<article class="change-item"><strong>${x.product_name} · ${x.robux} R$</strong><p>${x.buyer_name} · ${x.time}</p></article>`).join('')||'<p class="muted">Keine Verkäufe.</p>';}
