...
def verify_order_page():
    ...
    result = db.verify_order_public(oid) if oid else None
    return Response(pages.render_verify_page(oid, result), mimetype="text/html")


@app.route("/leaderboard")
def leaderboard_page():
    leaders = db.get_hall_of_fame()
    return Response(pages.render_leaderboard_page(leaders), mimetype="text/html")


@app.route("/api/magic_push", methods=["POST"])
def api_magic_push():
    data = request.json or {}
    user = data.get("user")
    product = data.get("product", "infinityxeh")
    if not user:
        return jsonify({"status": "error"}), 400
    if bot_instance and bot_instance.is_ready():
        import asyncio
        from bot.cogs.tickets import trigger_magic_push
        fut = asyncio.run_coroutine_threadsafe(trigger_magic_push(bot_instance, user, product), bot_instance.loop)
        try:
            res = fut.result(timeout=5)
            return jsonify(res)
        except Exception:
            pass
    db.add_web_order(user, product, "magic_push")
    return jsonify({"status": "success", "channel": "Discord Chat"})
...
def api_admin_products():
    ...
    for k, p in products_for_web().items():
        products.append({
            ...
            "delivery_msg": overrides.get(k, {}).get("delivery_msg", ""),
        })
...
def api_admin_product_update(product_key):
    ...
    if "delivery_msg" in data: db.set_product_override(product_key, "delivery_msg", str(data["delivery_msg"]))
    if "robux" in data:
...
def api_admin_flashsale():
    ...
    if not active:
        with db.lock:
            db.data["flash_sale"] = {"active": False}
        db.save()
        return jsonify({"status": "stopped"})
    prod = data.get("product", "anti_ban")
    discount = int(data.get("discount", 25))
    minutes = int(data.get("minutes", 30))
    import datetime
    ends_dt = datetime.datetime.utcnow() + datetime.timedelta(minutes=minutes)
    ends_iso = ends_dt.strftime("%Y-%m-%d %H:%M:%S")
    db.set_flash_sale(prod, discount, ends_iso)
    return jsonify({"status": "started", "flash_sale": db.get_flash_sale()})
