...
# Session-Secret für Admin-Login (in Railway als SECRET_KEY setzen)
app.secret_key = os.environ.get("SECRET_KEY", "void-shop-default-secret-change-me")
# Admin-Passwort (in Railway als ADMIN_PASSWORD setzen!)
...
<body>
  ...
  <div class="grid">
    <div class="card"><div><div class="card-title">♾️ INFINITYxEH</div><div class="card-desc">Premium All-in-One Executor — INFINITY × EH Edition.</div><div class="price-tag">750 R$ <span style="font-size:1rem; color:#a0aec0;">/ 7,50 €</span></div></div><div><button class="btn-buy" onclick="openModal('INFINITYxEH', 'discord')">Über Discord kaufen</button><button class="btn-coins" onclick="openModal('INFINITYxEH', 'coins')">Mit Void-Coins bezahlen (750 🪙)</button></div></div>
    <div class="card"><div><div class="card-title">💉 FFlags Injector</div><div class="card-desc">Automatischer FastFlag-Injector für maximale FPS & Performance.</div><div class="price-tag">300 R$ <span style="font-size:1rem; color:#a0aec0;">/ 3,00 €</span></div></div><div><button class="btn-buy" onclick="openModal('FFlags Injector', 'discord')">Über Discord kaufen</button><button class="btn-coins" onclick="openModal('FFlags Injector', 'coins')">Mit Void-Coins bezahlen (300 🪙)</button></div></div>
    <div class="card"><div><div class="card-title">🛡️ Anti-Ban</div><div class="card-desc">Schutzsystem gegen Bans — sicher, stabil und regelmäßig geupdatet.</div><div class="price-tag">1.000 R$ <span style="font-size:1rem; color:#a0aec0;">/ 10,00 €</span></div></div><div><button class="btn-buy" onclick="openModal('Anti-Ban', 'discord')">Über Discord kaufen</button><button class="btn-coins" onclick="openModal('Anti-Ban', 'coins')">Mit Void-Coins bezahlen (1000 🪙)</button></div></div>
    <div class="card"><div><div class="card-title">👕 T-Shirt Templates</div><div class="card-desc">50+ Roblox T-Shirt Vorlagen mit Verkaufsrechten, PNG/PSD Dateien.</div><div class="price-tag">500 R$ <span style="font-size:1rem; color:#a0aec0;">/ 5,00 €</span></div></div><div><button class="btn-buy" onclick="openModal('T-Shirt Templates', 'discord')">Über Discord kaufen</button><button class="btn-coins" onclick="openModal('T-Shirt Templates', 'coins')">Mit Void-Coins bezahlen (500 🪙)</button></div></div>
    <div class="card"><div><div class="card-title">🚀 FastFlags Pack</div><div class="card-desc">Ultra FPS-Boost Config mit stabilen Roblox FastFlags und Setup-Hilfe.</div><div class="price-tag">150 R$ <span style="font-size:1rem; color:#a0aec0;">/ 1,50 €</span></div></div><div><button class="btn-buy" onclick="openModal('FastFlags Pack', 'discord')">Über Discord kaufen</button><button class="btn-coins" onclick="openModal('FastFlags Pack', 'coins')">Mit Void-Coins bezahlen (150 🪙)</button></div></div>
    <div class="card"><div><div class="card-title">🖥️ Discord Server Template</div><div class="card-desc">Premium Discord-Shop Template mit Rollen, Kanälen und Ticket-Struktur.</div><div class="price-tag">400 R$ <span style="font-size:1rem; color:#a0aec0;">/ 4,00 €</span></div></div><div><button class="btn-buy" onclick="openModal('Discord Server Template', 'discord')">Über Discord kaufen</button><button class="btn-coins" onclick="openModal('Discord Server Template', 'coins')">Mit Void-Coins bezahlen (400 🪙)</button></div></div>
  </div>
...
# ==================================================================
# ADMIN-BEREICH (Passwort-Login)
# ==================================================================
LOGIN_HTML = """<!DOCTYPE html><html lang="de"><head><meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>𝗩𝗢𝗜𝗗 • Admin Login</title><style>
*{box-sizing:border-box;margin:0;padding:0;font-family:'Segoe UI',Roboto,sans-serif}
body{background:#0b0d14;color:#f0f4f8;min-height:100vh;display:flex;align-items:center;justify-content:center;
background-image:radial-gradient(circle at 20% 20%,rgba(0,240,255,.08),transparent 40%),radial-gradient(circle at 80% 80%,rgba(255,0,127,.08),transparent 40%)}
.box{background:rgba(16,20,32,.9);border:1px solid rgba(0,240,255,.3);border-radius:20px;padding:2.5rem;width:340px;box-shadow:0 10px 40px rgba(0,0,0,.6)}
h1{font-size:1.5rem;margin-bottom:.4rem;background:linear-gradient(90deg,#00f0ff,#ff007f);-webkit-background-clip:text;-webkit-text-fill-color:transparent}
p{color:#a0aec0;font-size:.9rem;margin-bottom:1.5rem}
input{width:100%;padding:.9rem;border-radius:12px;border:1px solid rgba(255,255,255,.15);background:#11141f;color:#fff;margin-bottom:1rem;font-size:1rem}
button{width:100%;padding:.9rem;border:none;border-radius:12px;background:linear-gradient(90deg,#00f0ff,#ff007f);color:#fff;font-weight:700;font-size:1rem;cursor:pointer}
.err{color:#ff4757;font-size:.85rem;margin-bottom:1rem;min-height:1rem}
</style></head><body><form class="box" method="post" action="/admin/login">
<h1>𝗩𝗢𝗜𝗗 • Admin</h1><p>Bitte melde dich an, um das Dashboard zu sehen.</p>
...
def admin_login():
    if request.method == "POST":
        ...
        return Response(LOGIN_HTML.replace("__ERR__", "❌ Falsches Passwort!"), mimetype="text/html")
    if is_logged_in():
        return redirect("/admin")
...
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>𝗩𝗢𝗜𝗗 • Admin Dashboard</title><style>
*{box-sizing:border-box;margin:0;padding:0;font-family:'Segoe UI',Roboto,sans-serif}
body{background:#0b0d14;color:#f0f4f8;padding:1.5rem;min-height:100vh;
background-image:radial-gradient(circle at 10% 10%,rgba(0,240,255,.07),transparent 40%),radial-gradient(circle at 90% 90%,rgba(255,0,127,.07),transparent 40%)}
header{display:flex;justify-content:space-between;align-items:center;border-bottom:2px solid rgba(0,240,255,.25);padding-bottom:1rem;margin-bottom:1.5rem;flex-wrap:wrap;gap:1rem}
h1{font-size:1.8rem;background:linear-gradient(90deg,#00f0ff,#ff007f);-webkit-background-clip:text;-webkit-text-fill-color:transparent}
.logout{background:rgba(255,71,87,.15);border:1px solid #ff4757;color:#ff4757;padding:.5rem 1rem;border-radius:10px;text-decoration:none;font-weight:600}
.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:1.4rem;margin-bottom:2rem}
.card{background:rgba(16,20,32,.85);border:1px solid rgba(0,240,255,.25);border-radius:18px;padding:1.5rem;position:relative;overflow:hidden}
.card::before{content:'';position:absolute;top:0;left:0;width:100%;height:4px;background:linear-gradient(90deg,#00f0ff,#ff007f)}
.label{font-size:.8rem;text-transform:uppercase;color:#a0aec0;letter-spacing:1px;margin-bottom:.5rem}
.value{font-size:2rem;font-weight:800}
.cyan{color:#00f0ff}.green{color:#39ff14}.pink{color:#ff007f}.gold{color:#ffd700}
.panel{background:rgba(16,20,32,.85);border:1px solid rgba(0,240,255,.25);border-radius:18px;padding:1.5rem;margin-bottom:1.5rem}
.panel h2{font-size:1.1rem;margin-bottom:1rem;color:#00f0ff}
.bar-row{display:flex;align-items:center;gap:.8rem;margin-bottom:.6rem}
.bar-label{width:60px;font-size:.85rem;color:#a0aec0}
.bar-track{flex:1;background:rgba(255,255,255,.06);border-radius:8px;height:22px;overflow:hidden}
.bar-fill{height:100%;background:linear-gradient(90deg,#00f0ff,#ff007f);border-radius:8px;display:flex;align-items:center;padding-left:8px;font-size:.75rem;font-weight:700;color:#fff;white-space:nowrap}
table{width:100%;border-collapse:collapse}
td,th{padding:.6rem;text-align:left;border-bottom:1px solid rgba(255,255,255,.08)}
th{color:#a0aec0;font-size:.8rem;text-transform:uppercase}
</style></head><body>
<header><h1>⚡ 𝗩𝗢𝗜𝗗 • Admin Dashboard</h1><a class="logout" href="/admin/logout">Abmelden</a></header>
<div class="grid" id="kpis"></div>
<div class="panel"><h2>📈 Umsatz (letzte 7 Tage)</h2><div id="chart"></div></div>
<div class="panel"><h2>🏆 Tickets pro Supporter</h2><div id="supporters"></div></div>
<div class="panel"><h2>📦 Lagerbestand & Verkäufe pro Produkt</h2><table id="products"><thead><tr><th>Produkt</th><th>Lager</th><th>Verkäufe</th></tr></thead><tbody></tbody></table></div>
<script>
async function load(){
 const r=await fetch('/api/admin/stats');const d=await r.json();
 const kpi=[['💰 Umsatz gesamt',d.total_robux+' R$','gold'],['📅 Heute',d.today_robux+' R$','green'],
  ['🛒 Verkäufe',d.total_sales,'cyan'],['🏆 Bestseller',d.best_product,'pink'],
  ['🎟️ Tickets',d.total_tickets,'cyan'],['⏱️ Ø Antwort',d.avg_response,'green'],
  ['⭐ Zufriedenheit',d.satisfaction+'/5','gold']];
 document.getElementById('kpis').innerHTML=kpi.map(k=>`<div class="card"><div class="label">${k[0]}</div><div class="value ${k[2]}">${k[1]}</div></div>`).join('');
 const max=Math.max(1,...d.timeseries.map(t=>t[1]));
 document.getElementById('chart').innerHTML=d.timeseries.map(t=>`<div class="bar-row"><div class="bar-label">${t[0]}</div><div class="bar-track"><div class="bar-fill" style="width:${Math.max(3,t[1]/max*100)}%">${t[1]} R$</div></div></div>`).join('');
 const sm=Math.max(1,...Object.values(d.per_supporter));
 document.getElementById('supporters').innerHTML=Object.entries(d.per_supporter).map(([n,c])=>`<div class="bar-row"><div class="bar-label">${n}</div><div class="bar-track"><div class="bar-fill" style="width:${Math.max(5,c/sm*100)}%">${c}</div></div></div>`).join('')||'<p style="color:#a0aec0">Noch keine Daten</p>';
 document.querySelector('#products tbody').innerHTML=d.products.map(p=>`<tr><td>${p.name}</td><td>${p.stock}</td><td>${p.sold}</td></tr>`).join('');
}
...
def api_admin_stats():
    ...
    for k in PRODUCT_ORDER:
        ...
        products.append({
            ...
            "stock": db.stock_count(k),
            "sold": s["per_product"].get(p["name"], 0),
        })
    return jsonify({
        ...
        "timeseries": db.sales_timeseries(7),
        "per_supporter": t["per_supporter"],
        "products": products,
    })
